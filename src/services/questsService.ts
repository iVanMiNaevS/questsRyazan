import {IQuestData, IQuestFull} from "@/types/quests.interface";
...
export const questsService = {
	...
	getQuestBySlug: async (slug: string): Promise<ServiceResponse<IQuestFull>> => {
		try {
			if (!process.env.NEXT_PUBLIC_QUEST_API_TOKEN) {
				throw new Error("API token is not configured");
			}

			const response = await fetch(`${apiUrl}/api/Quest/getBySlug?slug=${slug}`, {
				headers: {
					Authorization: process.env.NEXT_PUBLIC_QUEST_API_TOKEN,
				},
				next: {revalidate: 3600},
			...
			const quest: IQuestFull = await response.json();
			return {data: quest, message: "ok", ok: true};
		} catch (error) {
			let errorMessage = "Unknown error occurred";
